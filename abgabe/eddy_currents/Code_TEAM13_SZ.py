...
Omega = {'air':1, 'coil':2, 'conductor':3}
...
class CurrentExpression(UserExpression):
  def __init__(self, size, j = 1.0968, center = None, **kwargs):
    super().__init__()
    self._j = j
    self._center = center
    self._size = size
...
j_expr = CurrentExpression(size=np.array([100,100,0]), center=np.array([0, 0, 0]),\
    j=2742./(0.1*0.025), degree=1)
j_expr = interpolate(j_expr, VectorFunctionSpace(mesh, "CG", 1))

a = inner(curl(u),curl(v))*dx # + 1e-9*inner(u,v)*dx
...
mu0 = 4.*np.pi*1e-7
mus = {1: mu0, 2: mu0, 3: 1e3*mu0}
mu = Function(V0)
mu.vector()[:] = np.array([mus[i] for i in domains.array()])

sigmas = {1: 1e-5, 2: 1e-5, 3: 1.01e9}
sigma = Function(V0)
...
a -= Constant(scale**2*omega)*sigma*inner(u_i, v_r)*dx #(Omega['plate'])
a -= Constant(scale**2*omega)*sigma*inner(u_r, v_i)*dx #(Omega['plate'])
